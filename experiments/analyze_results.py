...
def plot_bound_decomposition_from_yaml(decomposition_files, output_path, budget=None):
    """
    Create stacked bar chart showing bound decomposition for multiple allocation strategies.
    Uses YAML files generated by eval_bounds.py.

    Args:
        decomposition_files: Dict mapping allocation names to decomposition YAML file paths
        output_path: Path to save the plot
        budget: Intrinsic dimension (for title)
    """
    # Load decomposition data
    data = {}
    for name, filepath in decomposition_files.items():
        if os.path.exists(filepath):
            with open(filepath, 'r') as f:
                data[name] = yaml.safe_load(f)

    if not data:
        print("  No bound decomposition files found - skipping bound decomposition plot")
        return

    # Extract components for each allocation strategy
    allocations = list(data.keys())
    fp_bpd = []
    quant_loss = []
    complexity = []
    total_bounds = []
    ...
    for alloc in allocations:
        d = data[alloc]
        fp_bpd.append(d.get('fp_bpd_train', 0))
        quant_loss.append(d.get('quantization_loss', 0))
        complexity.append(d.get('complexity_term', 0))
        total_bounds.append(d.get('total_bound', 0))
    ...
    # Create stacked bars
    ax.bar(x, fp_bpd, width, label='FP BPD (Train)', color='#5B9BD5')
    ax.bar(x, quant_loss, width, bottom=fp_bpd, label='Quantization Loss', color='#C55A5A')

    # Calculate bottom for complexity term
    bottom_complexity = np.array(fp_bpd) + np.array(quant_loss)
    ax.bar(x, complexity, width, bottom=bottom_complexity, label='Complexity Term', color='#70AD47')

    # Add value labels on each segment
    for i, alloc in enumerate(allocations):
        # FP BPD label
        ax.text(i, fp_bpd[i]/2, f'{fp_bpd[i]:.2f}',
                ha='center', va='center', fontsize=11, fontweight='bold', color='white')

        # Quantization Loss label
        if quant_loss[i] > 0.1:
            ax.text(i, fp_bpd[i] + quant_loss[i]/2, f'{quant_loss[i]:.2f}',
                    ha='center', va='center', fontsize=11, fontweight='bold', color='white')

        # Complexity Term label
        ax.text(i, bottom_complexity[i] + complexity[i]/2, f'{complexity[i]:.2f}',
                ha='center', va='center', fontsize=11, fontweight='bold', color='white')

        # Total bound label on top
        ax.text(i, total_bounds[i] + 0.15, f'{total_bounds[i]:.2f}',
    ...
    for name in allocations:
        if 'uniform' in name.lower():
            display_names.append('Uniform')
        elif 'b_heavy' in name.lower() or 'bheavy' in name.lower():
            display_names.append('B-heavy')
        elif 'equal' in name.lower():
            display_names.append('Equal')
        elif 'a_heavy' in name.lower() or 'aheavy' in name.lower():
            display_names.append('A-heavy')
        elif 'learned' in name.lower():
            display_names.append('Learned')
        else:
            display_names.append(name.replace('_', ' ').title())

    ax.set_xticklabels(display_names, fontsize=12)

    # Color x-tick labels to match paper style
    colors_map = {'Uniform': 'black', 'B-heavy': '#4472C4', 'Equal': '#70AD47',
                  'A-heavy': 'black', 'Learned': '#7030A0'}
    for tick_label in ax.get_xticklabels():
        label_text = tick_label.get_text()
        if label_text in colors_map:
            tick_label.set_color(colors_map[label_text])
            tick_label.set_fontweight('bold')

    budget_str = f' (d={budget})' if budget else ''
    ax.set_title(f'Bound Decomposition: FP BPD + Quantization Loss + Complexity{budget_str}',
...
def main():
    ...
    # Extract metrics
    print("\n[1/14] Extracting bounds metrics from experiments...")
    ...
    print("\n[7/14] Generating learned gating trend plots...")
    ...
    print("\n[8/14] Generating gating time-series plots (per-iteration)...")
    ...
    print("\n[9/14] Generating prediction rank histograms (combining seeds)...")
    for budget in args.budgets:
        plot_topk_histogram(args.results_dir, budget, args.seeds, budget_dirs[budget], k_values=[100, 500, 1000])

    print("\n[10/14] Generating magnitude distribution plots (combining seeds)...")
    for budget in args.budgets:
        plot_magnitude_distribution(args.results_dir, budget, args.seeds, budget_dirs[budget])

    print("\n[11/14] Generating sparsity pattern plots (combining seeds)...")
    for budget in args.budgets:
        plot_sparsity_patterns(args.results_dir, budget, args.seeds, budget_dirs[budget], percentile_threshold=95)

    print("\n[12/14] Generating index stability analysis...")
    for budget in args.budgets:
        plot_index_stability_across_seeds(args.results_dir, budget, args.seeds, budget_dirs[budget], k=1000)

    # Generate bound decomposition plots from YAML (if files exist)
    print("\n[13/14] Generating bound decomposition plots (from YAML)...")
    for budget in args.budgets:
        # Auto-discover bound decomposition YAML files
        pattern = os.path.join(args.results_dir, f'**/bound_decomposition_d{budget}_*.yml')
        ...
        if yaml_files:
            ...
            for filepath in yaml_files:
                ...
                # Extract allocation name from filename
                allocation_name = filename.replace(f'bound_decomposition_d{budget}_', '').replace('.yml', '')
                decomposition_files[allocation_name] = filepath

            # Sort allocations in preferred order
            preferred_order = ['uniform', 'b_heavy', 'equal', 'a_heavy', 'learned']
            sorted_allocations = [a for a in preferred_order if a in decomposition_files]
            # Add any remaining allocations not in preferred order
            for alloc in decomposition_files:
                if alloc not in sorted_allocations:
                    sorted_allocations.append(alloc)

            # Reorder dictionary
            decomposition_files = {k: decomposition_files[k] for k in sorted_allocations if k in decomposition_files}

            output_path = os.path.join(budget_dirs[budget], 'bound_decomposition_yaml.png')
            plot_bound_decomposition_from_yaml(decomposition_files, output_path, budget)
        else:
            print(f"  No bound decomposition YAML files found for d={budget} - skipping")

    # Generate summary table
    print("\n[14/14] Generating summary tables...")
